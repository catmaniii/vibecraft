...
另外还有一件**已知要来、但还没来**的事：**SC2 5.0.16（"八农民版本"）会让现有 build 全部失准。**

那个补丁 2026-06-22 上线，是 11 年来最大的一次平衡改动。和 build 直接相关的：

| 改动 | 从 → 到 |
|---|---|
| **开局农民** | **12 → 8** |
| Nexus / 指挥中心 提供人口 | 15 → 13 |
| Hatchery 提供人口（造价） | 6 → 4（275 → 300） |
| 大矿 / 小矿 | 1800 → 1600 / 900 → 1100 |
| 气矿（每基地总气） | 2250 → 2500（4500 → 5000） |
| 折跃门 | 研究后 BG 生产快 40%；转换需 25/25；warp-in 统一 4 秒 |

**为什么这是全量返工而不是微调**：本项目所有 build 的步骤都写成 `<supply> build ...`。
开局 8 农民 + 主基地只给 13 人口，意味着**同一个 supply 数字对应的游戏状态完全变了**——
4BG 的 `13 build BG`，旧版是"开局 12 农民之后再补 1 个"，新版成了"8 农民之后补 5 个"。
48 个剧本 + 47 份验收 spec 的每一步都要重新校准。折跃门改动还会单独影响所有神族
warp-in 相关的 timing（`4bg` / `iac_2base` / chargelot 产线等）。

顺带一个前置隐患：`python-sc2`（burnysc2）的版本表只认到 **5.0.14**（base 94137）。
客户端更新到 5.0.16 后，可能要先补版本表才能起局。

**这不是一次性工作，是持续维护**——以后每次平衡补丁都要走一遍全量重跑。
...
**现状**：访问控制只有一个**房间 token**，而且它就写在 URL 里（`?room=<token>`）——
拿到链接的人就能给 bot 下指令、看你的游戏画面。链接一旦被转发、截图、贴进聊天，
就等于把钥匙给出去了。（admin 面板另有一个独立 token，那个能改服务端设置。）

**现在还能接受**，是因为服务器基本靠口口相传：不知道地址就连不上。
但**方向 C（服务器目录）会把这层遮挡拿掉**——服务器一旦公开可发现，
"没人知道我的地址"就不再是防线了。
...
| 做法 | 好处 | 代价 |
|---|---|---|
| 维持现状（一个共享 token） | 零摩擦，朋友之间发个链接就能玩 | 链接即钥匙，无法单独踢人、无法追责 |
| 一次性 / 限时邀请链接 | 挡住链接被无限转发 | 主机每次都要重新生成、发给人 |
| 轻量账号（昵称 + 口令 / OAuth） | 能认人、能踢人、能记录 | 要存用户数据，责任和复杂度都上一个台阶 |

**倾向**：这是个玩朋友局的工具，不是公共服务，**过度设计鉴权可能把最大的优点（打开链接就能玩）
弄没了**。但做方向 C 之前得先想清楚。有想法欢迎开 Issue 讨论。
