...
![手机横屏驾驶舱：左边 SC2 实时画面，右边宏观策略 / 战术决策 / 运营策略面板，底部指令输入](docs/images/cockpit-live.jpg)

*手机横屏驾驶舱实拍：左边 SC2 实时画面（点哪都能拖镜头），右边从上到下是**宏观策略**（这局玩家
选的是「凤凰控场」，连 doctrine 参数一起摊开给你看）、**BOT 战术决策**（它此刻在开矿，还告诉你
为什么）、**运营策略**（采矿/农民这些常驻开关随手改）；底部一行是科技 / 产能 / 兵种一览 + 指令
