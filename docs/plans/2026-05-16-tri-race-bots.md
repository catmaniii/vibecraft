...
VoiceCraft 目前只有神族 bot（`_VoiceCraftBot` 继承裸 `AresBot`）。
本 plan 拆成三步：
...
**完成。** 从 `ares_adapter.py` 抽出到 `src/voicecraft/bot/auto_combat/common.py`：

- `_log_move_camera_done(task)` —— move_camera done callback
- `build_role_map()` —— 构造 voicecraft UnitRole → ares AresUnitRole 映射表（lazy import，ares 装了才能调）
- `run_command_with_echo(director, text, now, echo_callback)` —— echo 辅助协程，echo_callback 改为显式参数（原版是闭包捕获）

`ares_adapter.py` 的 `make_bot_class` 加 `race: str = "Protoss"` 参数；末尾 dispatch：`Protoss` → `_VoiceCraftBot`；其他 → `NotImplementedError`。
...
**voicecraft 原映射 `LLM_CONTROLLED → CONTROL_GROUP_ONE` 冲突**。

**决策**：改为 `LLM_CONTROLLED → CONTROL_GROUP_THREE`（Aristaeus 未使用）。
位置：`src/voicecraft/bot/auto_combat/common.py` 的 `build_role_map()`。

### config["Builds"] 注入

Aristaeus 的 `config.yml` / `protoss_builds.yml` 通过 ares 标准路径加载；
`bot/main.py` 没有自己写 `config["Builds"]` 注入逻辑。
voicecraft 的注入用 `.update()`，会覆盖/合并，不会被 Aristaeus 竞争。

### import 解决方案

`vendor/aristaeus/` 不在 Python import 路径里。选择 **sys.path 注入**：
在 `_VoiceCraftProtossBot` 模块顶部用 `importlib.util` 或直接 `sys.path.insert`，
在运行时把 `vendor/aristaeus` 注入到 `sys.path`。

具体实现：`src/voicecraft/bot/auto_combat/protoss/bot.py` 顶部做一次
...
voicecraft 覆写 `on_step` 时先 `await super().on_step(iteration)`（走 Aristaeus 逻辑），
再加 voicecraft 的 down_q 消费 + minimap 推送 + director.on_tick + facade.drain。

因此 **voicecraft 不再自行注册** Mining / AutoSupply / BuildWorkers 等 macro behavior；
...
| `src/voicecraft/bot/auto_combat/common.py` | CONTROL_GROUP_THREE 修改 |
| `src/voicecraft/bot/auto_combat/protoss/__init__.py` | 新建 |
| `src/voicecraft/bot/auto_combat/protoss/bot.py` | 新建 `_VoiceCraftProtossBot` |
| `src/voicecraft/bot/ares_adapter.py` | Protoss dispatch 指向新类 |
