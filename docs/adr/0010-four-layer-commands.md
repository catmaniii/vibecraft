...
| **L2 战术指令**（不指单位）| 进攻二矿 / 探中场 / 凤凰骚扰对面 | 一次性（完成/失败/超时）|
| **L3 standing order**（单位/建筑持久行为）| 3 凤凰巡逻二矿 / 2 追猎 hold 桥头 | 持久（玩家撤销 / 单位全死）|
...
| **P6** | 收尾：测试 + headless 验证（inject「切 4BG，打到对方二矿 OR 损失 70% 撤」）+ 本 ADR 补 corner case | 0.5d | P5 + P4 |
...
- **selector 不带 count**：边界 case 4 "3 个凤凰巡逻二矿" 教 LLM 用
