...
| # | name | 注入 | 预期 directive | verify | PASS 条件具体描述 |
|---|---|---|---|---|---|
| 3 | L2a tactical_attack | `进攻对方自然` | `tactical_objective(verb=attack, target_area=enemy_natural, done_when=any_of([target_destroyed, own_army_size_ratio]))` | `active_tactics` | snapshot `active_tactics` list 非空 或 events 含 `directive.committed` |
