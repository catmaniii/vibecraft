...
class ForwardSupportPylonGateway(ActBase):  # type: ignore[misc]
    ...
    async def start(self, knowledge: Any) -> None:
        ...
        try:
            ...
            logger.info(
                "ForwardSupport proxy=%s hide=%s", self.proxy_location, self.hide_location
            )
    ...
    async def execute(self) -> bool:
        ...
        try:
            ...
            py_at_proxy = self.ai.structures(UnitTypeId.PYLON).closer_than(
                14, self.proxy_location
            )
            ...
            if not self.retreating and ratio < self.RETREAT_RATIO:
                ...
                logger.debug(
                    "ForwardSupport retreating to hide (hp ratio=%.2f)", ratio
                )
