"""vibecraft OpeningBuild → ares config["Builds"] 翻译层。
...
- `abort_signals` / `default_transitions` / `phases` 归 vibecraft Board/DSL（M2+）。
"""

from __future__ import annotations

from vibecraft.strategy.models import BuildStep, OpeningBuild
...
# ares build runner 的 opening 期背景行为参数。vibecraft 剧本 steps 只列关键
...
def translate_opening_to_ares_steps(opening: OpeningBuild) -> list[str]:
    """vibecraft OpeningBuild 的 steps + scout_at → ares build order step 字符串列表。
