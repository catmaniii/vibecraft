...
class TaskMonitor:
    ...
    def __init__(self, board: Any, event_bus: EventBus) -> None:
        ...
        # 2026-05-24:类比 unit_built,structure_count_built_since 的 per-type 计数
        # 订阅 BUILDING_COMPLETE event,按 structure_type filter 累加。
        self._structure_built_counts: dict[str, dict[str, int]] = {}
        ...
        # 2026-05-24 用户:被指令的单位 tag set(用于 unit_arrived/unit_held_position
        # checker 拿位置)。director.attach_directive 时传入。
        self._directive_unit_tags: dict[str, set[int]] = {}
        # directive_id → unit_held_position 进入范围首 ts (None = 当前帧不全员在范围)
        self._held_first_in_range_ts: dict[str, float | None] = {}
    ...
    def attach_directive(
        ...
        issued_at: float,
        timeout_s: int | None,
        unit_tags: set[int] | None = None,
    ) -> None:
        """directive 进 board 时调, 设 listener + 状态初始化。

        done_when 接受 pydantic model (有 model_dump) 或 dict，向后兼容。
        unit_tags(2026-05-24): unit_arrived/unit_held_position checker 用,
        director 在 _maybe_attach_task_monitor 时从 _standing_order_tags 传入。
        """
        # pydantic → dict
        ...
        if unit_tags:
            self._directive_unit_tags[directive_id] = set(unit_tags)
    ...
    def _attach_subscriptions(
        ...
        # 2026-05-24: structure_count_built_since: 订阅 BUILDING_COMPLETE,
        # 按 structure_type filter 累加 per-type counter(类比 unit_count_built_since)
        elif kind == "structure_count_built_since":
            st = done_when.get("structure_type")
            counter_key = st or "*"
            self._structure_built_counts.setdefault(directive_id, {}).setdefault(counter_key, 0)

            def _s_handler(event: Event, _did: str = directive_id, _key: str = counter_key) -> None:
                self._structure_built_counts[_did][_key] = (
                    self._structure_built_counts[_did].get(_key, 0) + 1
                )

            s_filter_fn: Callable[[Event], bool] | None
            if st is not None:

                def _s_filter(e: Event, _st: str = st, _iat: float = issued_at) -> bool:
                    if e.owner != "own" or e.ts < _iat:
                        return False
                    e_st = (e.unit_type or "").rsplit(".", 1)[-1].upper()
                    return e_st == _st.upper()

                s_filter_fn = _s_filter
            else:

                def _s_filter_any(e: Event, _iat: float = issued_at) -> bool:
                    return e.owner == "own" and e.ts >= _iat

                s_filter_fn = _s_filter_any

            sub_id = self.event_bus.subscribe(
                EventKind.BUILDING_COMPLETE,
                _s_handler,
                filter=s_filter_fn,
            )
            self._sub_ids[directive_id].append(sub_id)
    ...
    def detach(self, directive_id: str) -> None:
        ...
        self._structure_built_counts.pop(directive_id, None)
        ...
        self._directive_unit_tags.pop(directive_id, None)
        self._held_first_in_range_ts.pop(directive_id, None)
...
@register("structure_count_built_since")
def _check_structure_count_built_since(
    done_when: dict[str, Any],
    directive_id: str,
    game_state: Any,
    monitor: TaskMonitor,
    ...
    """done_when: {kind, structure_type, op, value}

    monitor._structure_built_counts[did][structure_type] 由 BUILDING_COMPLETE
    event handler 维护(类比 unit_count_built_since)。
    structure_type=None 时用 "*" key 表示"任意建筑"。
    """
    counts = monitor._structure_built_counts.get(directive_id, {})
    counter_key = done_when.get("structure_type") or "*"
    count = counts.get(counter_key, 0)
    value = int(done_when.get("value", 1))
    op = done_when.get("op", ">=")
    return _compare(count, op, value)
...
# 到达半径分级(2026-06-06 用户):大区域 named_spot(主矿/分矿/基地一带)锚点是一个点
# 但实际范围很大 → 用大半径;精确点(X后面/choke/ramp/瞭望塔/坐标)用小半径。code 侧兜底,
# 不靠 LLM 每次把 within_grid 蒙对。
_ARRIVAL_TOLERANCE_WIDE = 16.0
_ARRIVAL_TOLERANCE_TIGHT = 5.0
# 精确点关键字(优先级高于大区域:含这些即使带 main/natural 也算精确点)
_PRECISE_SPOT_KEYWORDS = (
    "back",
    "behind",
    "后",
    "ramp",
...
def _arrival_tolerance(area: Any, llm_within: float) -> float:
    """按 named_spot 类别返回到达半径。大区域宽、精确点窄;未知 → LLM within_grid 作下限。"""
    name = str(area or "").strip().lower()
    if name.startswith("("):
        return _ARRIVAL_TOLERANCE_TIGHT  # 坐标字符串 = 精确点
    if any(k in name for k in _PRECISE_SPOT_KEYWORDS):
        return _ARRIVAL_TOLERANCE_TIGHT
    if any(k in name for k in _WIDE_SPOT_KEYWORDS):
        return _ARRIVAL_TOLERANCE_WIDE
    return max(float(llm_within), _ARRIVAL_TOLERANCE_TIGHT)


@register("unit_arrived")
def _check_unit_arrived(
    done_when: dict[str, Any],
    directive_id: str,
    game_state: Any,
    monitor: TaskMonitor,
    ...
    """done_when: {kind, area, within_grid} — 指令单位**重心**距 area < 半径 → done。

    2026-05-24 用户:move/safe_move 自动完成判定。
    2026-06-06 用户:改判队伍重心进圈(不再要求每个掉队单位都进),半径按点位分级
    (大区域宽/精确点窄)。否则虚空散开到"对方主矿"这种大区域,锚点 5 格内永远凑不齐
    全部单位 → done_when 永不满足 → 赖到 timeout(虚空 dancing 关联根因)。
    """
    if game_state is None:
        return False
    tags = monitor._directive_unit_tags.get(directive_id)
    if not tags:
        return False
    area = done_when.get("area", "")
    point = _resolve_named_spot(area, game_state)
    if point is None:
        return False
    within = _arrival_tolerance(area, float(done_when.get("within_grid", 5.0)))
    try:
        units = game_state.units.tags_in(tags)
    except Exception:
        return False
    if not units:
        # 全死/不在场 → done(没必要继续)
...
@register("unit_held_position")
def _check_unit_held_position(
    done_when: dict[str, Any],
    directive_id: str,
    game_state: Any,
    monitor: TaskMonitor,
    ...
    """done_when: {kind, area, within_grid, hold_seconds} — selector 内全部单位
    在 area within_grid 内持续 hold_seconds 秒 → done。

    实现:跟 vision_acquired 类似,wall-clock ts diff 累计。中断重置。
    """
    if game_state is None:
        return False
    tags = monitor._directive_unit_tags.get(directive_id)
    if not tags:
        return False
    point = _resolve_named_spot(done_when.get("area", ""), game_state)
    if point is None:
        return False
    within = float(done_when.get("within_grid", 5.0))
    hold_seconds = float(done_when.get("hold_seconds", 5.0))
    try:
        units = game_state.units.tags_in(tags)
    except Exception:
        return False
    ...
    if units:
        try:
            all_in_range = all(u.distance_to(point) < within for u in units)
        except Exception:
            all_in_range = False
    if all_in_range:
        if monitor._held_first_in_range_ts.get(directive_id) is None:
            monitor._held_first_in_range_ts[directive_id] = now
        elapsed = now - monitor._held_first_in_range_ts[directive_id]  # type: ignore[operator]
        return elapsed >= hold_seconds
    monitor._held_first_in_range_ts[directive_id] = None
    return False
...
def _check_structure_count(
    ...
    """done_when: {kind, structure_type, op, value} — 己方已**建好**建筑数。

    2026-05-25 bug 8 修复:从 .amount + already_pending(含 building / pending)
    改成 .ready.amount。玩家"修两个 BF"期望"两个建好",不是"两个开始建"。
    Worker 刚开始建 Forge 时 .amount=N+building 数,如果按这个算,
    建造刚开始(几秒后)就 done → 卡片立刻消失,但实际建筑 ~45s 才建好。
    ...
    if type_id is None:
        return False
    try:
        current = game_state.structures(type_id).ready.amount
    except Exception:
        return False
