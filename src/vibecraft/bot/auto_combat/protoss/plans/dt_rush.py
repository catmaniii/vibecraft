...
极速 VB（DarkShrine），~4:10 首波 3 DT 到达对方主基地偷家：
  - 早期：速 BY + 折跃 + VT + VB
  - 出击：3 DT + Warp Prism 运载（效率更高）或直接走
  - 安全底：2 补 BG + 二矿，若 DT 被克（Detection）立转中期

关键升级
========
1. WarpgateResearch（BY 一好立刻研，全程 chrono）
2. BlinkTech 不研（节省矿和时间，资源全给 VB）
3. （可选）ShadowStrike / 额外 DT 补充

Build 节奏（参考 spawningtool.com 47308 + 82515，~4:10 首波）
=============================================================
  0:14  BE（Pylon）
  0:35  BG（Gateway）
  ...
  2:36  VB（DarkShrine）
  2:48  BE + 二矿
  3:50  warp DT × 3（VB 完成立刻，chrono 出）
