...
class VibeCraftZoneAttack(PlanZoneAttack):  # type: ignore[misc]
    ...
    def _should_attack(self, power: Any) -> bool:
        """优先读 combat_intent_override（L2 tactical_objective）；
        再读 stance_override（L2 engagement_constraint）；最后透传给 sharpy 默认逻辑。

        两个字段独立：tactical_objective 写 combat_intent_override，
        engagement_constraint 写 stance_override，互不干扰。
        ...
        if intent in ("defend", "hold", "retreat", "vision"):
            return False
        stance = getattr(self.knowledge.vibecraft, "stance_override", None)
