...
def make_terran_bot_class(
    ...
    class _VibeCraftTerranBot(VibeCraftBotBase):  # type: ignore[misc,valid-type]
        ...
        async def create_plan(self) -> BuildOrder:
            ...
            try:
                from vibecraft.bot.auto_combat.terran.plans.sustain import TerranSustain

                sustain_inst = TerranSustain()
                sustain_plan = await sustain_inst.create_plan()
                logger.info("create_plan: TerranSustain 兜底 plan 装载成功")
            except Exception as exc:
                logger.warning("create_plan: TerranSustain 装载失败，用空 BuildOrder 兜底: %s", exc)
