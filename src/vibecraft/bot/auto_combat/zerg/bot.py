...
本文件是虫族薄壳，只保留：
  ...
  - create_plan()（虫族 IfElse 路由树 + ScoutOverlord + ZergSustain）
...
def make_zerg_bot_class(
    ...
    class _VibeCraftZergBot(VibeCraftBotBase):  # type: ignore[misc,valid-type]
        ...
        async def create_plan(self) -> BuildOrder:
            """M6.2b：虫族 IfElse 路由树 + ScoutOverlord + ZergSustain 兜底。"""
            import importlib
            import inspect
            ...
            scout_act: Any = None
            try:
                from vibecraft.bot.auto_combat.zerg.plans.scout_overlord import (
                    ScoutOverlord,
                )

                scout_act = ScoutOverlord()
            except Exception as exc:
                logger.warning("ScoutOverlord 装载失败: %s — 跳过通用探路层", exc)
