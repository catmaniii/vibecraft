...
def compute_enemy_composition_tags(
    ...
    # 反隐
    has_detection = any(enemy_summary.get(unit, 0) > 0 for unit in _DETECTOR_UNITS)
    ...
    if race == "zerg":
        ...
        if enemy_summary.get("Immortal", 0) >= 4 and enemy_summary.get("Colossus", 0) >= 2:
            tags.add("protoss_ground_mech")
        # 复合：地面无 storm
        if enemy_summary.get("Zealot", 0) >= 8 and "PsiStorm" not in enemy_upgrades:
        ...
        if enemy_summary.get("Battlecruiser", 0) >= 4 or enemy_summary.get("Banshee", 0) >= 6:
            tags.add("terran_sky")
        if marines >= 15 and enemy_summary.get("WidowMine", 0) >= 5:
            tags.add("terran_marine_widow")
        if enemy_summary.get("Thor", 0) >= 4 and marines >= 20 and medivacs >= 2:
