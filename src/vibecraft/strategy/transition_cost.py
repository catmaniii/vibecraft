...
def _unit_field_cost(unit_type: str) -> float:
    """单个单位的转型成本度量（mineral + gas×倍率）。

    Archon 特例：unit_data 里是 0/0（morph 无直接造价），但转型实际要付 2 个
    源单位的钱。HT 合成（2×HighTemplar）和 DT 合成（2×DarkTemplar）两条路线
    取便宜的那条。
    """
    if unit_type == "Archon":
        ht = get_unit_cost("HighTemplar")
        dt = get_unit_cost("DarkTemplar")
        via_ht = 2.0 * (ht.mineral + ht.gas * GAS_MULTIPLIER)
        via_dt = 2.0 * (dt.mineral + dt.gas * GAS_MULTIPLIER)
        return min(via_ht, via_dt)
    data = get_unit_cost(unit_type)
    return data.mineral + data.gas * GAS_MULTIPLIER
...
def transition_cost(
    ...
    for struct_type, target_count in target.required_structures.items():
        ...
        if missing > 0:
            ...
            build_cost += missing * (
                data.mineral + data.gas * GAS_MULTIPLIER + data.build_time * RAMP_FACTOR
            )
        ...
        for prereq in transitive_prereqs(struct_type):
            ...
            if game.structure_count(prereq) == 0:
                ...
                build_cost += (
                    pdata.mineral + pdata.gas * GAS_MULTIPLIER + pdata.build_time * RAMP_FACTOR
                )
    ...
    for upgrade in target.required_tech:
        if not game.has_upgrade(upgrade) and not game.is_researching(upgrade):
            ...
            tech_cost += (
                data.mineral + data.gas * GAS_MULTIPLIER + data.build_time * TECH_TIME_FACTOR
            )
    ...
    for unit_type, target_count in target.target_composition.items():
        ...
        missing = max(0, target_count - have)
        if missing > 0:
            unit_cost += missing * _unit_field_cost(unit_type)
    ...
    for unit_type, count in game.own_army_summary.items():
        if unit_type not in target_unit_set:
            obsolete_cost += count * _unit_field_cost(unit_type)
