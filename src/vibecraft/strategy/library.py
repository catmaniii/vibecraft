...
class StrategyLibrary:
    ...
    def __init__(
        ...
        # 2026-05-19 两层架构新增
        self._persistents: dict[str, PersistentDoctrine] = {s.id: s for s in (persistents or [])}
    ...
    def persistent_doctrines(self, race: str | None = None) -> list[PersistentDoctrine]:
        ...
        if race is None:
            return list(self._persistents.values())
        race_l = race.lower()
        return [d for d in self._persistents.values() if self._race_of.get(d.id) == race_l]
