...
// set_by 来源对应的中文标签 + 颜色(玩家=绿,自动转=蓝,bot 默认=灰)
const setByBadge = computed(() => {
  if (!props.slot) return null
  const map: Record<string, { text: string; cls: string }> = {
    voice: { text: '👤 玩家', cls: 'bg-success/20 text-success border-success/40' },
    auto_transition: { text: '🤖 自动转', cls: 'bg-blue-500/20 text-blue-300 border-blue-500/40' },
    bot_internal: { text: '⚙️ bot 默认', cls: 'bg-muted/20 text-muted border-muted/40' },
    abort: { text: '⚠️ 应急转', cls: 'bg-danger/20 text-danger border-danger/40' },
  }
...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between mb-1 gap-1">
      ...
      <span v-if="allComplete" class="text-xs font-bold text-success">✓ 已完成</span>
      <span v-else-if="props.isActive" class="text-xs font-bold text-accent">当前</span>
    </div>
