...
// strategy id → 前端显示名（已知策略的本地化名称）
function getStrategyDisplay(id: string): string {
  const map: Record<string, string> = {
    persistent_skytoss: t('autoswitch.strategySkytoss'),
    persistent_brood_corruptor: t('autoswitch.strategyBroodCorruptor'),
    persistent_skyterran: t('autoswitch.strategySkyterran'),
  }
  return map[id] ?? id
}
...
function getReasonDisplay(reason: string): string {
  const map: Record<string, string> = {
    cancel_redirected: t('autoswitch.reasonCancel'),
    opening_completed: t('autoswitch.reasonOpening'),
    parse_fail_redirected: t('autoswitch.reasonParseFail'),
  }
...
const chosenDisplay = computed(() =>
  getStrategyDisplay(props.switchEvent?.payload.chosen_id ?? '')
)
const reasonDisplay = computed(() =>
  getReasonDisplay(props.switchEvent?.payload.reason ?? '')
)
const alternativesDisplay = computed(() => {
  const alts = props.switchEvent?.payload.alternatives ?? []
  return alts.slice(0, 2).map((a) => getStrategyDisplay(a.id)).join(' / ')
})
...
const titleDisplay = computed(() => {
  ...
  return swap === false ? t('autoswitch.titleRecommend') : t('autoswitch.titleSwitched')
})
</script>
...
<template>
  ...
  >
    <!-- 2026-05-23 用户:从全局 fixed 浮层 → 改"当前宏观策略"框内 absolute overlay。
         父容器(CockpitView.vue 当前宏观策略框)需 position: relative。
         absolute inset-0 占满父框,backdrop-blur 让下层 StrategyCard 隐约可见。-->
    <div
      v-if="visible && switchEvent != null"
      data-testid="auto-switch-toast"
      class="absolute inset-0 z-10 flex flex-col justify-center
             rounded-xl bg-accent/15 border border-accent/50
    ...
    >
      ...
      <p class="text-xs text-white/80 mt-1">
        {{ t('autoswitch.reasonLabel') }} {{ reasonDisplay }}
        <span v-if="alternativesDisplay" class="text-white/50">
          {{ t('autoswitch.altLabel') }} {{ alternativesDisplay }}
        </span>
      </p>
    </div>
  </Transition>
