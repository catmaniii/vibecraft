...
<template>
  <div class="rounded-xl border border-border bg-surface-2 p-3">
    <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">{{ t('standing.title') }}</p>

    <!-- 空态 -->
    <p v-if="orders.length === 0" class="text-xs text-muted italic">{{ t('standing.empty') }}</p>
    ...
    <ul v-else class="space-y-1.5">
      ...
      >
        ...
        <button
          ...
          :aria-label="t('tactics.revokeAria', { label: order.display })"
