...
**新增 (Added)**：
- `docs/images/cockpit-live.jpg` —— 真局里手机横屏驾驶舱的截图，放在中英 README 开头
  一句话卖点之后。之前 README 全靠 ASCII 框图描述界面，第一次点进仓库的人**看不到它到底
  长什么样**；这张图一眼把"实时画面 + 宏观策略 / 战术决策 / 运营策略 + 指令输入"的布局说完。
- 图下配文点出了三块面板各自在干什么（宏观策略摊开 doctrine 参数、BOT 报告它此刻为什么在
  开矿、运营开关可随手改），并注明截图取自演示视频、所以带英文字幕。
- 图从原始 2584px 缩到 1920px 宽、JPEG q88（1.26 MB → 290 KB），面板小字仍可读。

**移除 (Removed)**：
- `docs/images/cockpit-demo.png` —— 2026-05 的界面草稿（画面里还是「等待小地图数据…」和几个
  虚线占位框），入库后**从未被任何文档引用过**，现在有真局实拍图了，这张过时占位图没有留的理由。
...
**新增 (Added)**：
- **查清并写进路线图：SC2 补丁 5.0.16（"八农民版本"）2026-06-22 上线，会让现有 build 全部失准。**
  这不是"需要迭代"级别的事，是**全量返工**：本项目所有 build 的步骤都写成 `<supply> build ...`，
  而开局农民 **12 → 8**、Nexus/CC 供给 **15 → 13**、Hatchery **6 → 4** 之后，**同一个 supply
  数字对应的游戏状态完全变了**——4BG 的 `13 build BG` 旧版是"12 农民之后再补 1 个"，
  新版成了"8 农民之后补 5 个"。48 个剧本 + 47 份验收 spec 每一步都要重新校准；折跃门 rework
  （研究后 BG 生产快 40%、转换 25/25、warp-in 统一 4 秒）还会单独影响所有神族 warp-in timing。
  矿/气也重新分配（大矿 1800→1600、小矿 900→1100、每基地总气 4500→5000）。
- **一并查出的前置隐患**：`python-sc2`（burnysc2）的版本表只认到 **5.0.14**（base 94137），
  而本机安装已是 base 96883、线上最新是 5.0.16（build 97364）。客户端更新后可能要先补版本表
  才能起局。
- **新增「开放问题」一节：鉴权要做多强？** 现状是访问控制只有一个房间 token 且写在 URL 里——
  拿到链接就能给 bot 下指令、看游戏画面。现在靠"不知道地址就连不上"兜着，但**方向 C
  （服务器目录）恰恰会把这层遮挡拿掉**。列了三种做法（维持现状 / 一次性邀请链接 / 轻量账号）
  各自的好处与代价，并写明倾向：这是朋友局工具不是公共服务，**过度设计鉴权可能把"打开链接
  就能玩"这个最大优点弄没**——但方向 C 落地前必须先定。中英两版同步。

### 2026-07-28 TASKS 重写成公开路线图（中英双语），个人信息与会话交接搬出仓库

**变更 (Changed)**：
- **`TASKS.md` 从"进展流水账"重写成"接下来要做什么"**，并新增英文版 `TASKS.en.md`。
  原文件约七成篇幅是已完成事项的记录，与 `CHANGELOG.md` 重复——按单一数据源纪律删掉、指过去。
  新结构：现状一览 → 三大方向 → 具体待办（按"要不要真机 SC2"分组）→ 已知问题 →
  **明确不做的** → 从哪下手（三条不同门槛的入口）。
- **三大方向刷新成真实状态**：
  - **A 多地图适配**（升为最卡的一个）：整个项目只在 DaybreakLE 上验证过；难点不是加载新图，
    是 named_spot、特色落点、地形阈值全和这张图绑死。
  - **B build 库持续优化**：修正了旧文档的说法——开局其实已经补齐（神 9 / 人 12 / 虫 9），
    真正的短板是**中后期 doctrine**（神 8，人虫各 5）。并写明 SC2 版本更新会让所有按 supply
    写的 timing 全部偏移，是**持续维护**而非一次性工作。
  - **C 多服务器（新）**：志愿者出自己的 Windows PC + 自己的 VPS、各自承担流量，
    维护者的 VPS 只跑一个**服务器目录**（注册/心跳/下线），玩家进首页看到服务器列表。
    关键是**目录本身流量极小**，费流量的控制面和媒体面各走各家 VPS —— 成本因此可以分摊。
- **旧文档里已被推翻的内容清掉**：「开源排除项：推理图谱不进开源交付物」（RG 现在就在公开仓库里，
  `/rg` 还是刻意公开的）、「v1.5 才加虫族/人族」（三族早就有了）、`--extra sc2` 相关的过时装法。

**安全 (Security)**：
- **个人信息从公开仓库抹掉**：`TASKS.md` 原有的「用户环境关键事实」写着维护者的 SC2 安装路径、
  开发用房间 token、GitHub 账号、手机测试习惯。仓库开源后这些没理由留在公网。
  连同会话交接状态一起搬进 **`SESSION.local.md`（已 gitignore，不入库）**。
- `CLAUDE.md` 的会话交接协议同步改写，并明确三份文件的分工：
  `SESSION.local.md`（不入库）= 停在哪 / 本机环境；`TASKS.md`+`TASKS.en.md`（公开）= 将来做什么；
  `CHANGELOG.md`（公开）= 已经做完什么。

### 2026-07-28 语言切换链接去掉国旗 + USER_GUIDE 中英互链

**变更 (Changed)**：
- 两个 README 顶部的语言切换去掉国旗 emoji，改成 `**中文** · [English](...)`（当前语言加粗、
  另一语言是链接）。用国旗代表语言本就不准确——旗子代表国家，不代表语言。
- **`USER_GUIDE.md` ↔ `USER_GUIDE_EN.md` 之前根本没有互链**：英文读者从 README.en 点进英文
  玩家手册后，没有任何回到中文版的入口。两边都补上同样样式的切换行。
...
**新增 (Added)**：
- **两个 README 的「自行部署」章开头加了 mermaid 拓扑图**，讲清**手机 / VPS / PC 三方关系**：
  实线=控制面（网页、信令、指令，流量极小），虚线=媒体面（画面和声音，PC→手机）。配三条要点：
  ①**server 永远和 SC2 同机、搬不到云上**（要启动游戏、按窗口 PID 抓那台机器的屏），云只干
  "把手机接到你 PC"这一件事；②**VPS 是可选的**（同 wifi / Tailscale 直连即可）；
  ③**媒体面尽量不走 VPS**（WebRTC 先试 P2P，打不通才落 TURN 中继，所以 VPS 带宽花不了多少）。
  - 验证不是"看着对"：临时装了真的 mermaid 11 解析器（配 jsdom 补 DOM，否则 DOMPurify 报
    `purify.addHook is not a function`），对两个 README 里的块跑 `mermaid.parse()` —— 都通过。

**变更 (Changed)**：
- **推理图谱 `is_worker` 那批过了独立 subagent 复评**（365 → 372 节点）。复评抓到的核心问题是
  **两个承重的隐藏前提被塞在从句和 evidence 里、没有节点承载**，机器沿边扫不到：
  - 新增 **F180**「单帧异常被 on_step 顶层兜底吞掉后本局继续」—— 没有它，"直接访问抛异常"
    的结论应该是"整局崩"而不是"静默失效"，整条推理其实缺了一环。
  - 新增 **F182**「修复前的单测 mock 自己给单位造了 is_worker 属性」+ **I70**「行为单测挡不住
    这类假属性 bug」—— 这是"为什么要加静态审计"的唯一理由，此前只写在 evidence 的破折号后面。
  - 拆开多 claim 的节点：F176（属性不存在）/ F178（没有 `__getattr__` 兜底）各自独立——两者是
    两条不同分支的前提，捆一起的话上游哪天加了 `__getattr__` 只能整条撤回；F177（16 处）/
    F179（12 处 getattr + 4 处直接访问）同理。
  - D115 拆成 D115（判据统一走 type_id）+ D116（加静态审计），两者可独立采纳/撤销。
  - **F181 那组实测数字换成能指到 log 的**：原写"修前 0/163、修后 66/133"，但现存 log 里找不到
    对应（应是某次一次性自验的即时计数）。改用多局可复核的计数并逐个标出文件：修复前
    `nydus24.out` 393 次打击 0 个农民、`wave8r.out` 478/0、`server_20260726_235046.log` 72/0；
    修复后 `server_20260727_035105.log` 288/107（37%）、`server_20260727_080542.log` 468/162（35%）。
    证据反而比原来更硬 —— 这正是 `rg_audit` 的「实测 fact 的 evidence 没指到具体 log」提示逼出来的。
- 复评水位盖到 `2026-07-28`，增量复审归零。
