...
class TestPayloads:
    ...
    def test_self_cast_burrow_target_kind(self) -> None:
        """2026-06-17:寡妇雷埋地等自施法 target.kind='self' 必须能过校验。

        真局 match_20260617_000202:LLM 给 BURROWDOWN_WIDOWMINE 吐 target.kind='self'
        (自施法本就无外部目标),但枚举漏了 self → schema 拒 → 玩家看到"识别失败"。
        复刻当时被拒的 payload,确认现在能过。
        """
        # TargetKind.SELF 存在
        assert TargetKind.SELF.value == "self"
        # 复刻真局 call_005 被拒的整条 payload
        payload = UnitClaimPayload.model_validate(
            {
                "selector": {"unit_type": "WidowMine"},
                "task": {
                    "primary_action": {
                        "verb": "cast_ability",
        ...
        assert payload.task.primary_action.target.kind == TargetKind.SELF
        assert payload.task.primary_action.ability_id == "BURROWDOWN_WIDOWMINE"
...
class TestSubmitAndCommit:
    ...
    def test_tick_commits_pending(self) -> None:
        # 2026-05-26 默认 delay=0,这里显式传 1.5 保留"延迟机制"覆盖。
        board = DirectiveBoard(commit_delay_s=1.5)
        d = _strategy_set("opening", "1g_robo", issued_at=10.0)
...
class TestStandingOrderSchema:
    ...
    def test_selector_count_field(self) -> None:
        """2026-05-24 用户:selector 加 count 字段(玩家"一个农民"→count=1 防全锁)。"""
        s = Selector(unit_type="Probe", count=1)
        assert s.count == 1
        s2 = Selector(unit_type="Probe")  # 默认 None(不限)
        assert s2.count is None
        # count >= 1
        with pytest.raises(ValidationError):
            Selector(unit_type="Probe", count=0)
