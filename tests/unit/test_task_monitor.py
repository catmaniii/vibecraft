...
# ---------------------------------------------------------------------------
# unit_arrived: 队伍重心进圈 + 半径按点位分级 (2026-06-06 用户)
# ---------------------------------------------------------------------------


class _FakePos(tuple):
    """最小 Point2 替身:支持 distance_to + 下标。"""
...
class _FakeUnits:
    ...
    def tags_in(self, tags: set) -> _FakeUnits:
        return _FakeUnits([u for u in self._u if u.tag in tags])
    ...
    @property
    def center(self) -> _FakePos:
        n = len(self._u)
        return _FakePos(
            (sum(u.position[0] for u in self._u) / n, sum(u.position[1] for u in self._u) / n)
        )


def _arrived_setup(monkeypatch, spot_point, units):
    """构造 monitor + game_state,patch _resolve_named_spot 返回 spot_point。"""
    from vibecraft.bot import task_monitor as tm

    monkeypatch.setattr(tm, "_resolve_named_spot", lambda name, gs: spot_point)
    monitor = TaskMonitor(board=MagicMock(), event_bus=EventBus())
    monitor._directive_unit_tags["d1"] = {u.tag for u in units}
    gs = MagicMock()
    gs.units = _FakeUnits(units)
    return monitor, gs


class TestUnitArrivedCenter:
    def test_arrival_tolerance_levels(self) -> None:
        """大区域(主矿/分矿)宽半径;精确点(后面/choke/ramp/瞭望塔)窄半径。"""
        from vibecraft.bot.task_monitor import _arrival_tolerance

        # 大区域
        assert _arrival_tolerance("enemy_main", 5.0) >= 12
        assert _arrival_tolerance("enemy_natural", 5.0) >= 12
        assert _arrival_tolerance("main", 5.0) >= 12
        # 精确点(即使含 main)
        assert _arrival_tolerance("enemy_main_back", 5.0) <= 6
        assert _arrival_tolerance("main_ramp", 5.0) <= 6
        assert _arrival_tolerance("watchtower_right", 5.0) <= 6
        assert _arrival_tolerance("(120.0, 30.0)", 99.0) <= 6  # 坐标=精确点,无视 LLM 大值

    def test_center_in_wide_radius_arrives_even_with_straggler(self, monkeypatch) -> None:
        """大区域:重心进圈即到达,允许个别单位掉队(不要求全部进圈)。"""
        from vibecraft.bot.task_monitor import _check_unit_arrived

        spot = _FakePos((100.0, 100.0))
        units = [
            _FakeUnit(1, 100.0, 100.0),
            _FakeUnit(2, 105.0, 100.0),
            _FakeUnit(3, 130.0, 100.0),  # 掉队单位,距锚点 30(旧逻辑会因它 FAIL)
        ]
        # 重心 ≈ (111.7,100),距 100 约 11.7 < enemy_main 宽半径(~16) → 到达
        monitor, gs = _arrived_setup(monkeypatch, spot, units)
        done_when = {"kind": "unit_arrived", "area": "enemy_main", "within_grid": 5.0}
        assert _check_unit_arrived(done_when, "d1", gs, monitor) is True

    def test_center_outside_radius_not_arrived(self, monkeypatch) -> None:
        """重心还没进圈 → 未到达。"""
        from vibecraft.bot.task_monitor import _check_unit_arrived

        spot = _FakePos((100.0, 100.0))
        units = [_FakeUnit(1, 60.0, 100.0), _FakeUnit(2, 70.0, 100.0)]  # 重心 (65,100) 距 35
        monitor, gs = _arrived_setup(monkeypatch, spot, units)
        done_when = {"kind": "unit_arrived", "area": "enemy_main", "within_grid": 5.0}
        assert _check_unit_arrived(done_when, "d1", gs, monitor) is False

    def test_precise_point_tight_radius(self, monkeypatch) -> None:
        """精确点(主矿后面)用窄半径:重心差 10 格不算到。"""
        from vibecraft.bot.task_monitor import _check_unit_arrived

        spot = _FakePos((100.0, 100.0))
        units = [_FakeUnit(1, 110.0, 100.0)]  # 距 10 > 精确点窄半径(~5)
        monitor, gs = _arrived_setup(monkeypatch, spot, units)
        done_when = {"kind": "unit_arrived", "area": "enemy_main_back", "within_grid": 5.0}
        assert _check_unit_arrived(done_when, "d1", gs, monitor) is False

    def test_all_units_dead_done(self, monkeypatch) -> None:
        """单位全死光 → done(units 空)。"""
        from vibecraft.bot.task_monitor import _check_unit_arrived

        spot = _FakePos((100.0, 100.0))
        monitor, gs = _arrived_setup(monkeypatch, spot, [])
        monitor._directive_unit_tags["d1"] = {1, 2}  # tags 非空但场上无单位
        done_when = {"kind": "unit_arrived", "area": "enemy_main", "within_grid": 5.0}
        assert _check_unit_arrived(done_when, "d1", gs, monitor) is True
