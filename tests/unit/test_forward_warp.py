...
def _make_instance(
    ...
    inst.ai = ai
    # sharpy knowledge mock — cooldown_manager 是 forward_warp 的 CD 判定来源
    # (`cm.is_ready(tag, ab, cooldown=23)` 走 sharpy used_dict 路径)。
    # 默认 is_ready=True(CD 已过,允许 warp);test 要测"CD 未到"就显式覆盖。
...
class TestExecuteShortCircuits:
    async def test_returns_false_when_minerals_zero(self):
        ...
        forward_wg = _make_struct((89.3, 29.7), UnitTypeId.WARPGATE)
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(warpgates=[forward_wg], pylons=[forward_py], minerals=0, vespene=0)
    ...
    async def test_supply_full_does_not_mark_cooldown(self):
        """supply 满时不调 warp_in 也不注册 cm.used_ability(防浪费 23s 窗口)。

        Regression: warp_in 在 supply 满时被 sc2 reject 不开 cooldown,
        但我们如果注册到 cm.used_ability 会阻塞 23s 后重试 — 即使 supply 后来够。
        """
        from sc2.ids.unit_typeid import UnitTypeId
        ...
        # 关键:不能注册 used_ability(否则 sharpy cd_manager 误判 CD 中)
        inst.knowledge.cooldown_manager.used_ability.assert_not_called()
...
class TestHardCooldownTracking:
    """ForwardWarpStalker CD 判定:走 sharpy `cm.is_ready(tag, ab, cooldown=23)` —
    sharpy 内部 used_dict 模式(不信 SC2 API 的 available_dict,因为它对 WG warp
    ability 不过滤 cooldown → 误报)。
    """

    async def test_recent_warp_blocks_next_call(self):
        """sharpy cm.is_ready=False(CD 未过) → 不调 warp_in。"""
        from sc2.ids.unit_typeid import UnitTypeId
        ...
        inst.ai.can_afford = MagicMock(return_value=True)
        inst.ai.time = 100.0
        # 模拟 CD 未过:cm.is_ready 返 False
        inst.knowledge.cooldown_manager.is_ready.return_value = False
    ...
    async def test_warp_allowed_after_cooldown(self):
        """cm.is_ready=True(CD 已过) → 允许 warp + 注册 used_ability。"""
        from sc2.ids.unit_typeid import UnitTypeId
        from sc2.position import Point2
        ...
        inst = _make_instance(warpgates=[forward_wg], pylons=[forward_py])
        inst.ai.time = 130.0
        # _make_instance 默认 cm.is_ready=True;此处显式表达意图
        inst.knowledge.cooldown_manager.is_ready.return_value = True
        ...
        # warp 成功后注册到 sharpy used_dict
        inst.knowledge.cooldown_manager.used_ability.assert_called_once()
...
class TestMultiWarpResourceTracking:
    ...
    async def test_warps_four_when_all_resources_sufficient(self):
        ...
        wgs = [_make_struct((89, 30), UnitTypeId.WARPGATE, tag=i + 100) for i in range(4)]
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(
            warpgates=wgs,
            pylons=[forward_py],
    ...
    async def test_warps_two_when_only_money_for_two(self):
        ...
        wgs = [_make_struct((89, 30), UnitTypeId.WARPGATE, tag=i + 200) for i in range(4)]
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(
            warpgates=wgs,
            pylons=[forward_py],
    ...
    async def test_each_warpgate_gets_unique_spot(self):
        ...
        wgs = [_make_struct((89, 30), UnitTypeId.WARPGATE, tag=i + 400) for i in range(4)]
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(
            warpgates=wgs,
            pylons=[forward_py],
    ...
    async def test_no_warps_when_can_place_returns_all_false(self):
        ...
        wgs = [_make_struct((89, 30), UnitTypeId.WARPGATE, tag=i + 500) for i in range(2)]
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(
            warpgates=wgs,
            pylons=[forward_py],
    ...
    async def test_warps_three_when_supply_blocks_fourth(self):
        ...
        wgs = [_make_struct((89, 30), UnitTypeId.WARPGATE, tag=i + 300) for i in range(4)]
        forward_py = _make_struct((83.5, 28.5), UnitTypeId.PYLON)
        inst = _make_instance(
            warpgates=wgs,
            pylons=[forward_py],
