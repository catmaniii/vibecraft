...
class TestLoopSc2CrashCleanup:
    """_loop_sc2_crash_cleanup 单测：非 Windows 平台应立即 return（no-op）。"""

    async def test_noop_on_non_windows(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """非 Windows 平台（sys.platform!='win32'）调用后立即返回，不阻塞。"""
        monkeypatch.setattr("sys.platform", "linux")
        svc = BotService(ServiceConfig(token="test-cleanup"))
        # 直接 await 协程——非 Windows 下应瞬间 return，不进 while True 循环
        coro = svc._loop_sc2_crash_cleanup()
        result = await asyncio.wait_for(coro, timeout=1.0)
        assert result is None  # 协程正常结束（return None）
