...
class TestLLMConfig:
    def test_from_yaml_loads_fields(self) -> None:
        """从**仓库内的模板** config/llm.yaml.example 加载，字段全正确。

        2026-07-27 开源前修:原来读的是 `config/llm.yaml` —— 那是 **gitignore 掉的本地私有配置**
        (可能含明文 key)。CI 上根本没这个文件、每个贡献者机器上的值也不一样,这条测试于是既
        测不到仓库真实内容、又会在别人机器上随机红。改读入库的模板。
        """
        cfg = LLMConfig.from_yaml(PROJECT_ROOT / "config" / "llm.yaml.example")
    ...
    def test_build_provider_yaml_api_key_used(self) -> None:
        """yaml 里写明文 api_key → 无 env / 无显式参数时用它（key 可配置）。"""
        cfg = LLMConfig(provider="anthropic", api_key="sk-yaml-key")
        ...
        with (
            ...
            cfg.build_provider()
            mock_cls.assert_called_once_with(api_key="sk-yaml-key")

    def test_build_provider_yaml_api_key_overrides_env(self) -> None:
        """yaml api_key 优先于环境变量。"""
        cfg = LLMConfig(provider="anthropic", api_key="sk-yaml-key")
        with (
            patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-env-key"}),
            patch("anthropic.AsyncAnthropic") as mock_cls,
    ...
    def test_build_provider_explicit_key_overrides_yaml(self) -> None:
        """显式参数 > yaml api_key > env（最高优先级是显式参数）。"""
        cfg = LLMConfig(provider="anthropic", api_key="sk-yaml-key")
        with (
            patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-env-key"}),
            patch("anthropic.AsyncAnthropic") as mock_cls,
