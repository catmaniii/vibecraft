...
class TestCreatePlan:
    ...
    async def test_create_plan_builds_ifl_else_for_multiple_recipes(self) -> None:
        ...
        plan = await instance.create_plan()
        # BuildOrder 包含 IfElse（args[0] 是 FakeIfElse 或含 FakeIfElse 的 list）
        assert isinstance(plan, FakeBuildOrder)
        assert len(plan.args) > 0
        inner = plan.args[0]  # IfElse(cond_a, plan_a, plan_b) or [IfElse, sustain_act, ...]

        # 两个 recipe → 有 IfElse（嵌套）或直接是 FakeBuildOrder（单 recipe 时）
        # _wrap 现在可能把 [plan, sustain_act] 作为 list 传入 BuildOrder
        def _contains_plan_type(x: Any) -> bool:
            if isinstance(x, (FakeIfElse, FakeBuildOrder)):
                return True
            if isinstance(x, list):
                return any(isinstance(item, (FakeIfElse, FakeBuildOrder)) for item in x)
            return False

        assert _contains_plan_type(inner), (
            f"expected IfElse or BuildOrder (or list containing them), got {type(inner)}: {inner}"
        )
    ...
    async def test_set_build_then_lambda_condition_matches(self) -> None:
        ...
        plan = await instance.create_plan()
        # _wrap 现在把 [plan, sustain_act] 作为 list 传入 BuildOrder;
        # plan.args[0] 可能是 FakeIfElse 本身或含 FakeIfElse 的 list
        raw_inner = plan.args[0]
        if isinstance(raw_inner, list):
            # 找出 list 中的 FakeIfElse
            ifelse_items = [x for x in raw_inner if isinstance(x, FakeIfElse)]
            assert ifelse_items, f"expected FakeIfElse in list, got {raw_inner}"
            inner = ifelse_items[0]
        else:
            inner = raw_inner
