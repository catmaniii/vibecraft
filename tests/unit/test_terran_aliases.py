...
class TestTerranAliasResolve:
    ...
    def test_bc_resolves_to_battlecruiser(self, terran_alias_table: any) -> None:
        """'BC' → Battlecruiser（战巡通用缩写；2026-06-18 修:原错配 FusionCore，
        BC=战巡是社区通用,聚变芯 hotkey 是 VC）。"""
        canonical, _ = terran_alias_table.resolve("BC")
        assert canonical == "Battlecruiser"

    def test_vc_resolves_to_fusion_core(self, terran_alias_table: any) -> None:
        """'VC' → FusionCore（聚变芯 hotkey V+C，CLAUDE.md 真值表）。"""
        canonical, _ = terran_alias_table.resolve("VC")
