...
class TestRoomRegistry:
    ...
    def test_attach_first_connection_evicts_nothing(self) -> None:
        r = RoomRegistry()
        c1 = _FakeConn()
        assert r.attach(c1, player_id="p1") is None
        assert r.connection_of("p1") is c1

    def test_attach_second_connection_evicts_first(self) -> None:
        r = RoomRegistry()
        c1, c2 = _FakeConn(), _FakeConn()
        r.attach(c1, player_id="p1")
        evicted = r.attach(c2, player_id="p1")
        assert evicted is c1
        assert r.connection_of("p1") is c2
    ...
    def test_detach_clears_active(self) -> None:
        ...
        r.attach(c1, player_id="p1")
        r.detach(c1)
        assert r.connection_of("p1") is None

    def test_detach_stale_connection_does_not_clear_new(self) -> None:
        r = RoomRegistry()
        c1, c2 = _FakeConn(), _FakeConn()
        r.attach(c1, player_id="p1")
        r.attach(c2, player_id="p1")  # c1 被顶掉
        r.detach(c1)  # c1 的延迟断开
        assert r.connection_of("p1") is c2  # 新连接没被误清
