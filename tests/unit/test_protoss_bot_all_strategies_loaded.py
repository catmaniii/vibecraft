...
# 期望被加载的神族策略 id（8 opening + persistent doctrine；新增 doctrine 时补这里）
_EXPECTED_STRATEGY_IDS: set[str] = {
    ...
    "void_ray_rush",
    "persistent_skytoss",
    "persistent_colossus_immortal",
    "persistent_immortal_archon",
    "persistent_blink_harass",
    "persistent_phoenix_control",
    "persistent_skytoss_no_ht",
    "persistent_colossus_no_ht",
    "persistent_immortal_archon_no_ht",
}


def _load_real_library():
    """加载 strategies/protoss/ 下所有真实 yaml。"""
...
def test_strategy_library_loads_all_strategies() -> None:
    """前置条件：strategy library 能加载全部神族策略且 id 集合精准匹配。"""
...
def test_all_strategies_have_sharpy_dummy_class() -> None:
    """所有策略都必须有 sharpy_dummy_class 字段，否则 bot.create_plan 会 silent skip。"""
    lib = _load_real_library()
    missing: list[str] = []
...
def test_bot_create_plan_loads_all_strategies_into_ifelse_tree() -> None:
    """bot.create_plan() 完整链路：全部策略 import + instantiate + 拼 IfElse 都成功。
    ...
    inst = BotClass()
    # 调 create_plan(KnowledgeBot 子类的 async 方法)。asyncio.run 自带 loop，
    # 不依赖全局 current loop（get_event_loop 已废弃 + 会被别的测试污染）。
