...
class TestIsDone:
    ...
    def test_worker_building_detects_pylon_order(self):
        """worker 正在 build Pylon → _worker_building(PYLON) 应返回 True。

        Regression：python-sc2 的 UnitOrder 字段是 `ability: AbilityData`，
        AbilityId 在 `.ability.id`（写 `.ability_id` 会 AttributeError）。
        """
        ...
        # 不应抛 AttributeError，且识别出正在建 Pylon
        assert inst._worker_building(worker, UnitTypeId.PYLON) is True
        # 未在建 Gateway
        assert inst._worker_building(worker, UnitTypeId.GATEWAY) is False
...
class TestPickProxyLocation:
    async def test_picks_valid_point_no_enemy_vision(self):
        inst = _make_proxy_instance()
        from sc2.position import Point2
    ...
    async def test_returns_None_when_no_enemy(self):
        ...
        try:
            await inst._pick_proxy_location()
        except Exception:
            pytest.fail("_pick_proxy_location should not raise on missing enemy_start")

    async def test_sharpy_fallback_when_all_filtered(self):
...
# ============================================================================
# Tests: 构造参数（generalize 到 production_type / production_count）
# ============================================================================


class TestConstructorVariants:
    def test_default_is_single_gateway(self):
        from sc2.ids.unit_typeid import UnitTypeId

        from vibecraft.bot.auto_combat.protoss.plans import forward_proxy

        inst = forward_proxy.ForwardSupportPylonGateway()
        assert inst.production_type == UnitTypeId.GATEWAY
        assert inst.production_count == 1

    def test_build_gateway_false_is_pylon_only(self):
        from vibecraft.bot.auto_combat.protoss.plans import forward_proxy

        inst = forward_proxy.ForwardSupportPylonGateway(build_gateway=False)
        assert inst.production_count == 0

    def test_stargate_variant(self):
        from sc2.ids.unit_typeid import UnitTypeId

        from vibecraft.bot.auto_combat.protoss.plans import forward_proxy

        inst = forward_proxy.ForwardSupportPylonGateway(
            production_type=UnitTypeId.STARGATE, production_count=2
        )
        assert inst.production_type == UnitTypeId.STARGATE
        assert inst.production_count == 2


# ============================================================================
# Tests: planner 接入（Pylon upfront + 生产建筑 post-pylon，power 约束）
# ============================================================================


class TestPlannerIntegration:
    async def test_pick_proxy_and_plan_uses_pylon_footprint(self, monkeypatch):
        """_pick_proxy_and_plan 用 planner 规划 1 个 Pylon 落点（footprint=2）。"""
        from sc2.ids.unit_typeid import UnitTypeId
        from sc2.position import Point2
        ...
        async def fake_plan(ai, anchor, building, count, **kwargs):
            calls.append((building, count, kwargs.get("footprint"), kwargs.get("power_source")))
            return [Point2((anchor.x, anchor.y))]

        monkeypatch.setattr(forward_proxy, "plan_building_cluster", fake_plan)

        inst = _make_proxy_instance()
        inst.ai = _make_mock_ai(enemy_main=(120, 30))
        result = await inst._pick_proxy_and_plan()
        assert result is not None
        _anchor, pylon_spot = result
        assert isinstance(pylon_spot, Point2)
        # 只规划了 Pylon（footprint=2），没有 power_source（Pylon 不需要能量）
        assert calls, "planner should be called"
        building, count, footprint, power = calls[0]
        assert building == UnitTypeId.PYLON
        assert count == 1
        assert footprint == 2
        assert power is None

    async def test_plan_production_passes_power_constraint(self, monkeypatch):
        """_plan_production 用 power_source=Pylon + power_radius=6.5 约束生产建筑。"""
        from sc2.ids.unit_typeid import UnitTypeId
        from sc2.position import Point2
        ...
        async def fake_plan(ai, anchor, building, count, **kwargs):
            ...
            captured["power_source"] = kwargs.get("power_source")
            captured["power_radius"] = kwargs.get("power_radius")
            captured["footprint"] = kwargs.get("footprint")
            return [Point2((anchor.x + 3, anchor.y)), Point2((anchor.x - 3, anchor.y))]
        ...
        inst.ai = _make_mock_ai()
        pylon_pos = Point2((120, 50))
        await inst._plan_production(pylon_pos)

        assert captured["building"] == UnitTypeId.STARGATE
        assert captured["count"] == 2
        assert captured["power_source"] == pylon_pos
        assert captured["power_radius"] == 6.5
        assert captured["footprint"] == 3
        assert inst._prod_spots is not None
        assert len(inst._prod_spots) == 2

    async def test_plan_production_none_sets_fallback(self, monkeypatch):
        """规划失败（None）→ _prod_fallback=True（不卡死，走 find_placement）。"""
        from sc2.position import Point2
        ...
        inst = forward_proxy.ForwardSupportPylonGateway()
        inst.ai = _make_mock_ai()
        await inst._plan_production(Point2((120, 50)))
        assert inst._prod_fallback is True
        assert inst._prod_spots is None


# ============================================================================
# Tests: 完成判定 for N 个生产建筑（stargate 变体）
# ============================================================================


class TestDoneStargateVariant:
    def _mk_struct(self, tag, type_id, pos, ready=True):
        from sc2.position import Point2
        ...
        s.distance_to = lambda other, _p=Point2(pos): _p.distance_to(
            other.position if hasattr(other, "position") else other
        )
        return s

    def test_not_done_with_only_one_stargate(self):
        from sc2.ids.unit_typeid import UnitTypeId
        from sc2.position import Point2

        inst = _make_proxy_instance()
        inst.production_type = UnitTypeId.STARGATE
        inst.production_count = 2
        py = self._mk_struct(100, UnitTypeId.PYLON, (120, 50), ready=True)
        vs1 = self._mk_struct(101, UnitTypeId.STARGATE, (122, 52), ready=False)
        inst.ai = _make_mock_ai(structures_by_tag={100: py, 101: vs1})
        inst.proxy_location = Point2((120, 50))
        inst._start_time = 0.0
        inst.ai.time = 10.0
        # pylon ready + 只有 1 个 VS → hwm 1 < 2 → 未完成
        assert not inst._is_done()

    def test_done_with_two_stargates(self):
        from sc2.ids.unit_typeid import UnitTypeId
        from sc2.position import Point2
        ...
        py = self._mk_struct(100, UnitTypeId.PYLON, (120, 50), ready=True)
        vs1 = self._mk_struct(101, UnitTypeId.STARGATE, (122, 52), ready=False)
        vs2 = self._mk_struct(102, UnitTypeId.STARGATE, (118, 52), ready=False)
        inst.ai = _make_mock_ai(structures_by_tag={100: py, 101: vs1, 102: vs2})
        inst.proxy_location = Point2((120, 50))
        inst._start_time = 0.0
        inst.ai.time = 10.0
        # pylon ready + 2 个 VS 落地（含在建）→ 完成
        assert inst._is_done()
