...
class TestBuildComponent:
    ...
    def test_prereq_chain_counted(self) -> None:
        ...
        target_just_fb = _make_doctrine(
            "fb", required_structures={"FleetBeacon": 1}
        )
        ...
        game_with_prereqs = GameSnapshot(
            structures={
                "Nexus": 1, "Gateway": 1, "CyberneticsCore": 1, "Stargate": 1,
            }
        )
...
class TestTechComponent:
    ...
    def test_already_researched_zero(self) -> None:
        ...
        assert transition_cost(target, game_done, set()) < transition_cost(target, game_missing, set())
    ...
    def test_in_progress_research_zero(self) -> None:
        ...
        game_done = GameSnapshot(upgrades={"Charge"})
        # 正在研究的不算缺
        assert transition_cost(target, game_in_progress, set()) == transition_cost(target, game_done, set())
...
class TestUnitComponent:
    ...
    def test_already_have_units_zero(self) -> None:
        ...
        game_empty = GameSnapshot()
        assert transition_cost(target, game_full, set()) < transition_cost(target, game_empty, set())
...
class TestGasBottleneck:
    ...
    def test_no_bottleneck_when_gas_sufficient(self) -> None:
        ...
        assert transition_cost(target, game_plenty, set()) < transition_cost(target, game_scarce, set())
...
class TestCounterComponent:
    ...
    def test_multiple_counter_hits_stack(self) -> None:
        ...
        assert transition_cost(target, GameSnapshot(), triple_hit) < transition_cost(target, GameSnapshot(), single_hit)
...
class TestObsoleteComponent:
    def test_obsolete_units_add_cost(self) -> None:
        ...
        game_empty = GameSnapshot()
        # stalker 沉没成本 → 加分
        assert transition_cost(target_skytoss, game_with_stalkers, set()) > transition_cost(target_skytoss, game_empty, set())
...
class TestWorkedExample:
    ...
    def _setup(self) -> tuple[GameSnapshot, set[str], StrategyLibrary]:
        ...
        game = GameSnapshot(
            structures={
                "Nexus": 2, "Pylon": 4, "Gateway": 1, "CyberneticsCore": 1,
                "Assimilator": 2, "RoboticsFacility": 1, "Forge": 1,
            },
            units={
                "Probe": 40, "Stalker": 3, "Sentry": 3, "Immortal": 5,
            },
        ...
        doctrines = [
            _make_doctrine(
                ...
                target_composition={"Immortal": 6, "Colossus": 4, "Stalker": 8, "Zealot": 8, "Sentry": 3},
                required_structures={"RoboticsFacility": 2, "RoboticsBay": 1, "TwilightCouncil": 1, "Forge": 1, "Nexus": 3, "Gateway": 5},
            ...
            _make_doctrine(
                ...
                target_composition={"Zealot": 16, "Immortal": 4, "Archon": 6, "HighTemplar": 4, "Sentry": 3},
                required_structures={"RoboticsFacility": 1, "TwilightCouncil": 1, "TemplarArchives": 1, "Gateway": 7, "Nexus": 3, "Forge": 1},
            ...
            _make_doctrine(
                "persistent_skytoss",
                target_composition={"Carrier": 12, "Tempest": 3, "HighTemplar": 5, "Mothership": 1, "Observer": 2},
                required_structures={"Stargate": 4, "FleetBeacon": 1, "TemplarArchives": 1, "TwilightCouncil": 1, "RoboticsFacility": 1, "Forge": 1, "Nexus": 4},
            ...
            _make_doctrine(
                ...
                required_structures={"TwilightCouncil": 1, "RoboticsFacility": 1, "Gateway": 6, "Nexus": 3},
            ...
            _make_doctrine(
                ...
                required_structures={"Stargate": 3, "FleetBeacon": 1, "TemplarArchives": 1, "TwilightCouncil": 1, "Nexus": 4, "Gateway": 4, "Forge": 1},
    ...
    def test_picks_low_transition_cost_doctrine(self) -> None:
        ...
        cheap_options = {"persistent_ground_mech", "persistent_iac_macro", "persistent_blink_harass"}
        ...
        assert chosen in cheap_options, (
            ...
            "all costs:\n"
            + "\n".join(f"  {k}: {v:.1f}" for k, v in sorted(all_costs.items(), key=lambda kv: kv[1]))
        )
