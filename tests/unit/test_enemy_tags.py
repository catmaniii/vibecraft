...
class TestComputeTagsGeneric:
    ...
    def test_no_detection_negated_when_raven_present(self) -> None:
        tags = compute_enemy_composition_tags({"Marine": 10, "Raven": 1}, enemy_race="terran")
...
class TestComputeTagsZerg:
    def test_ling_bane(self) -> None:
        tags = compute_enemy_composition_tags({"Zergling": 40, "Baneling": 10}, enemy_race="zerg")
        assert "zerg_ling_bane" in tags

    def test_ling_bane_below_ling_threshold(self) -> None:
        tags = compute_enemy_composition_tags({"Zergling": 20, "Baneling": 10}, enemy_race="zerg")
    ...
    def test_ultra_brood_composite(self) -> None:
        tags = compute_enemy_composition_tags({"Ultralisk": 4, "BroodLord": 4}, enemy_race="zerg")
    ...
    def test_zerg_race_required_for_specific_tags(self) -> None:
        ...
        tags = compute_enemy_composition_tags({"Zergling": 40, "Baneling": 10}, enemy_race=None)
        assert "zerg_ling_bane" not in tags
...
class TestComputeTagsProtoss:
    ...
    def test_skytoss_mothership(self) -> None:
        tags = compute_enemy_composition_tags({"Mothership": 1}, enemy_race="protoss")
        assert "protoss_skytoss" in tags

    def test_mothership_carrier_composite(self) -> None:
        tags = compute_enemy_composition_tags({"Carrier": 5, "Mothership": 1}, enemy_race="protoss")
    ...
    def test_dt(self) -> None:
        tags = compute_enemy_composition_tags({"DarkTemplar": 4}, enemy_race="protoss")
...
class TestComputeTagsTerran:
    def test_bio_medivac(self) -> None:
        tags = compute_enemy_composition_tags({"Marine": 30, "Medivac": 4}, enemy_race="terran")
    ...
    def test_mech_tank(self) -> None:
        tags = compute_enemy_composition_tags({"SiegeTank": 8, "Marine": 10}, enemy_race="terran")
        assert "terran_mech" in tags
        assert "terran_mech_tank" in tags

    def test_sky_terran_bc(self) -> None:
        tags = compute_enemy_composition_tags({"Battlecruiser": 5}, enemy_race="terran")
        assert "terran_sky" in tags

    def test_marine_widow(self) -> None:
        tags = compute_enemy_composition_tags({"Marine": 20, "WidowMine": 6}, enemy_race="terran")
        assert "terran_marine_widow" in tags

    def test_ghost(self) -> None:
        tags = compute_enemy_composition_tags({"Ghost": 4, "Marine": 10}, enemy_race="terran")
...
class TestEdgeCases:
    def test_unknown_unit_doesnt_crash(self) -> None:
        ...
        tags = compute_enemy_composition_tags({"NonExistentUnit": 100}, enemy_race="zerg")
        # 未知单位算入 ground supply（default supply=2），100 个 → 200 supply
        assert "mass_ground" in tags

    def test_unknown_race(self) -> None:
        tags = compute_enemy_composition_tags({"Marine": 50}, enemy_race="unknown")
        # 通用 tag 仍 work
        assert "mass_ground" in tags
        # 种族 tag 不出
    ...
    def test_enemy_upgrades_default(self) -> None:
        ...
        tags = compute_enemy_composition_tags({"Zealot": 20}, enemy_race="protoss")
        # 没 Charge → no_charge
        assert "protoss_no_charge" in tags
        assert "protoss_chargelot" not in tags

    @pytest.mark.parametrize("race", ["protoss", "zerg", "terran", None])
