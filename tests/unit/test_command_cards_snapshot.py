...
def _make_move_directive() -> Directive:
...
def _make_scout_directive() -> Directive:
    ...
    return Directive(payload=payload, issued_at=22.0)
...
class TestL2UnitDirectiveCommandCard:
    """2026-05-24 用户:单位级 directive(棱镜回基地/派农民侦查)需在卡片显示。"""

    def test_move_appears_as_l2_card(self, director: Director) -> None:
        """MOVE directive → L2 卡片含 type='move' + 中文 display。"""
        d = _make_move_directive()
        director._submit_directives([d], now=20.0)
        snap = director.build_snapshot(now=25.0)
        move_cards = [c for c in snap["command_cards"] if c.get("type") == "move"]
        assert len(move_cards) == 1
        card = move_cards[0]
        assert card["layer"] == "L2"
        assert "棱镜" in card["display"]
        assert "main" in card["display"] or "移动" in card["display"]

    def test_scout_appears_as_l2_card(self, director: Director) -> None:
        """SCOUT directive → L2 卡片含 type='scout' + 侦查 display。"""
        d = _make_scout_directive()
        director._submit_directives([d], now=22.0)
        snap = director.build_snapshot(now=25.0)
        scout_cards = [c for c in snap["command_cards"] if c.get("type") == "scout"]
        assert len(scout_cards) == 1
        card = scout_cards[0]
        assert card["layer"] == "L2"
        assert "侦查" in card["display"]
        assert "enemy_third" in card["display"]


class TestDoneGracePeriod:
    """2026-05-24 用户:完成后 5s grace 期内卡片仍显示(status='done','已完成')。"""

    def test_done_directive_stays_in_cards_with_done_status(self, director: Director) -> None:
        """模拟 directive 完成 → grace 期内 command_cards 仍含该卡 + status='done'。"""
        d = _make_production_override_directive()
        director._submit_directives([d], now=10.0)
        # 模拟 _release_directive_done 触发(实际生产中由 task_monitor done 或
        # _exec_structure_override all_ready 触发)
        director._release_directive_done(d, now=20.0, reason="done")

        snap = director.build_snapshot(now=22.0)
        # grace 期内(20s + 5s = 25s 之前)卡片仍显示
        cards = [c for c in snap["command_cards"] if c["id"] == d.id]
        assert len(cards) == 1
        assert cards[0]["status"] == "done"

    def test_done_directive_disappears_after_grace(self, director: Director) -> None:
        """grace 期(5s)过 → on_tick 真删 → 卡片消失。"""
        d = _make_production_override_directive()
        director._submit_directives([d], now=10.0)
        director._release_directive_done(d, now=20.0, reason="done")

        # tick at 26s(超 5s grace) → 真删
        director.on_tick(now=26.0)

        snap = director.build_snapshot(now=26.0)
        cards = [c for c in snap["command_cards"] if c["id"] == d.id]
        assert len(cards) == 0
