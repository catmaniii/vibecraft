...
class BuildGrid(Grid):
    ...
    def fill_line(self, ai, mining_filler, neutral_unit):
        ...
        # vibecraft: extend mineral-line exclusion to cover the full nexus→mineral
        # probe path (not just 4 steps from the patch).  Buildings placed in this
        # corridor block probe mining paths, so we mark the entire corridor as
        # InMineralLine before BuildingSolver assigns grid slots.
        # Walk from mineral toward nexus until 2 tiles before the expansion center
        # (the nexus itself is already marked as TownHall with a 5x5 blocker).
