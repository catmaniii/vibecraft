...
class MorphBaneling(MorphUnit):
    def __init__(self, target_count: int):
        super().__init__(
            UnitTypeId.ZERGLING,
            # vibecraft: 旧 ability MORPHZERGLINGTOBANELING_BANELING 在当前 SC2 数据里对小狗已失效、
            # 引擎静默丢弃 → 小狗永不变蛹（ling_bane 开局 plan 早发现并 workaround）。改用有效的
            # MORPHTOBANELING_BANELING，使 ZergUnit(BANELING) / build-aware sustain 能真正孵爆虫。
