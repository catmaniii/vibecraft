...
class Expand(ActBase):
    ...
    async def execute(self) -> bool:
        ...
        if _cap is not None:
            ...
            _stealth_zones = sum(
                1
                for z in self.zone_manager.our_zones_with_minerals
                if getattr(z, "our_townhall", None) is not None and z.our_townhall.tag in _stealth
            )
            if active_bases - _stealth_zones >= _cap:
                self.clear_worker()
                return True

        # vibecraft: 在建/待建偷矿基地也算进基地数（2026-06-12 用户）。玩家下了偷矿令但偷矿
        # 基地还没 ready（农民在路上 / Nexus 没好，不在 our_zones_with_minerals）时，bot 也当它
        # 是一片基地 → 延后自己开对应分矿（不开冗余分矿）。MINING（ready）的 cell 已计入
        # active_bases，stealth_pending_base_count 只数还没 ready 的，不重复。仅作用于 build 的
        # Expand(to_count) 判断（下面 active_bases >= to_count），不影响玩家开矿封顶（上面那段）。
        active_bases += int(getattr(_vc, "stealth_pending_base_count", 0) or 0)
