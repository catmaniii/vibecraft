...
class ActUnit(ActBase):
    ...
    def builders(self) -> Units:
        ...
        # vibecraft: 主力农民产线不在 stealth Nexus 造农民（stealth cell 自产；避免 bot 也在
        # 那造 + 双重生产）。仅对农民产线生效（worker type），其他产线不受影响。
        if self.unit_type == self.knowledge.my_worker_type:
            _vc = getattr(self.knowledge, "vibecraft", None)
            _stealth_th = getattr(_vc, "stealth_townhall_tags", None)
            if _stealth_th:
                _builders = _builders.filter(lambda b: b.tag not in _stealth_th)
    ...
    def is_done(self) -> bool:
        # vibecraft: 有偷矿基地时，主矿农民产到自己 ideal（满采）就停 —— 多产的农民没法转去
        # 偷矿基地（FENCE 隔离），只会在主矿堆着闲置。偷矿基地由 StealthCellManager 自产自补。
        # 主矿满采 = 非 stealth 基地 assigned_harvesters 之和 >= 它们 ideal 之和（随开矿/枯竭
        # 动态变）。主矿扩张 → ideal 涨 → 自动接着产。非偷矿局（集合空）不受影响。
        if self.unit_type == self.knowledge.my_worker_type:
            _stealth_th = getattr(
                getattr(self.knowledge, "vibecraft", None), "stealth_townhall_tags", None
            )
            if _stealth_th:
                try:
                    _main_th = [t for t in self.ai.townhalls.ready if t.tag not in _stealth_th]
                    _main_gas = [
                        g for g in self.ai.gas_buildings.ready if g.tag not in _stealth_th
                    ]
                    _ideal = sum(t.ideal_harvesters for t in _main_th) + sum(
                        g.ideal_harvesters for g in _main_gas
                    )
                    _assigned = sum(t.assigned_harvesters for t in _main_th) + sum(
                        g.assigned_harvesters for g in _main_gas
                    )
                    if _ideal > 0 and _assigned >= _ideal:
                        return True  # 主矿满采，别再多产农民闲置
                except Exception:
                    pass
        return self.get_unit_count() >= self.to_count

    async def execute(self) -> bool:
        # vibecraft: 产能封锁机制级拦截 —— 玩家"停止出X"后,在下训练指令前直接拦下。
        # knowledge.vibecraft.production_blocked = 被封锁兵种 canonical 名集合(Zealot/
        # Stalker..)。命中 → return True(当作已满足,不下令、不阻塞 build order 后续步骤)。
        # 比反应式"事后取消队列"主动:从机制上不让它进队列。覆盖 ActUnit(兵营/机械/星门
        # train)+ ProtossUnit 非折跃路径 + Zerg/Terran 单位(都走 ActUnit)。
        _vbc_blocked = getattr(getattr(self.knowledge, "vibecraft", None), "production_blocked", None)
        if _vbc_blocked and self.unit_type.name.upper() in {str(b).upper() for b in _vbc_blocked}:
            return True
