...
class PlanZoneDefense(ActBase):
    ...
    async def execute(self) -> bool:
        ...
        # vibecraft: 2026-06-17 玩家"全军防守"(combat_intent_override=="defend")时,PlanZoneDefense
        # **不再 claim/dispatch 主力**(非工人),主力防守定位完全交给 PlanZoneGather 的威胁感知锚点
        # (_vbc_threatened_zone:power>3.0 阈值 + 1.5x 滞回,无威胁回前沿点;单一稳定点)。
        # 根因:defend 下 PlanZoneDefense(主力→enemy_center 移动靶)与 PlanZoneGather(idle→稳定锚点)
        # 双目标 + role Idle↔Defending ~1Hz 翻转 → 大军每秒换目标、原地"保持队形拉扯"(真局复现:
        ...
        for i in range(0, len(self.zone_manager.expansion_zones)):
            ...
            if zone_defenders.exists or zone.is_ours or zone == self.zone_manager.own_main_zone:
                ...
                if vbc_defend:
                    # vibecraft: defend 下不 claim/dispatch 主力。释放残留的非工人 Defending
                    # (交还 gather,防切入 defend 那刻的残留 dispatch),并把附近 free 主力的 power
                    # **只计入 defenders 不 claim** —— 让下面 worker 逻辑知道"有兵在守",不 panic 拉农民。
                    if zone_defenders.exists:
                        self.roles.clear_tasks(zone_defenders)
                        for _t in list(zone_tags):
                            _u = self.cache.by_tag(_t)
                            if _u is not None and _u.type_id != self.worker_type:
                                zone_tags.remove(_t)
                    for unit in self.roles.free_units:
                        if self.unit_values.should_attack(unit) and (
                            unit.distance_to(zone.center_location) < zone.radius + 10
                        ):
